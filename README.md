...
브레드보드 전원 레일에 커패시터를 배치해 LED와 FND의 출력 상태가 바뀔 때 발생할 수 있는 순간적인 전압 변동을 완화했다.

해당 커패시터는 전원 레일의 비교적 느린 전압 변동을 완화하는 벌크 또는 평활 커패시터 역할을 한다. 74HC595의 스위칭 노이즈를 억제하기 위해서는 각 IC의 VCC-GND 핀 가까이에 0.1μF 세라믹 디커플링 커패시터를 별도로 배치하는 것이 적절하다.
